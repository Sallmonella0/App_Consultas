...
class AppGUI(ctk.CTk):
    def __init__(self, api):
        ...
        try:
            self.iconbitmap(r"E:\App_Consultas\logoGX.ico")  # Windows
        ...
        self.entry_id = ctk.CTkEntry(frame_top, width=200,
                                      fg_color="#222222",
                                      text_color="#D0F0C0",
                                      placeholder_text_color="#88CC88")
        self.entry_id.pack(side="left", padx=(0, 10))

        self.btn_consultar = ctk.CTkButton(frame_top, text="Consultar API",
                                           fg_color="#33CC33",
                                           hover_color="#22AA22",
                                           text_color="#111111",
                                           command=self.consultar_api_async)
        ...
        self.btn_csv = ctk.CTkButton(frame_export, text="Salvar CSV",
                                     fg_color="#33CC33",
                                     hover_color="#22AA22",
                                     text_color="#111111",
                                     command=self.exportar.salvar_csv)
        self.btn_csv.pack(side="left", padx=10)

        self.btn_excel = ctk.CTkButton(frame_export, text="Salvar Excel",
                                       fg_color="#33CC33",
                                       hover_color="#22AA22",
                                       text_color="#111111",
                                       command=self.exportar.salvar_excel)
        ...
        self.entry_filtro = ctk.CTkEntry(frame_filtro, width=150,
                                         fg_color="#222222",
                                         text_color="#D0F0C0",
                                         placeholder_text_color="#88CC88")
        self.entry_filtro.pack(side="left", padx=5)
        self.entry_filtro.bind("<KeyRelease>", lambda e: self.aplicar_filtro())

        self.combo_coluna = ctk.CTkComboBox(frame_filtro, values=COLUNAS, width=150,
                                            fg_color="#222222",
                                            text_color="#D0F0C0",
                                            button_color="#33CC33",
                                            button_hover_color="#22AA22")
        self.combo_coluna.set("PLACA")
        self.combo_coluna.pack(side="left", padx=5)

        self.btn_filtrar = ctk.CTkButton(frame_filtro, text="Aplicar filtro",
                                         fg_color="#33CC33",
                                         hover_color="#22AA22",
                                         text_color="#111111",
    ...
    def consultar_api(self):
        ...
        try:
            dados = self.api.consultar(id_msg)  # Seu método da API
            self.after(0, lambda: self.tabela.atualizar(dados))
            if not dados:
                self.after(0, lambda: messagebox.showinfo("Aviso", f"Nenhum dado encontrado para IDMENSAGEM {id_msg}."))
        except Exception as e:
    ...
    def aplicar_filtro(self):
        ...
        self.tabela.filtrar(termo, coluna)
