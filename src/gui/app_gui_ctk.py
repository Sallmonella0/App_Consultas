...
from src.utils.settings_manager import AUTO_REFRESH_MINUTES
...
class AppGUI(ctk.CTk):
    def __init__(self, api):
        ...
        self.geometry(self.app_state.get("geometry", "1200x650"))
        ...
        self.btn_limpar_filtro = ctk.CTkButton(self.frame_bottom, text="Limpar Filtro", width=100, command=lambda: self.limpar_filtro())
    ...
    def carregar_dados_iniciais_com_cache(self, is_auto_refresh=False):
        try:
            if not is_auto_refresh:
                ...
                if dados_iniciais:
                    self.after(0, self.tabela.atualizar_dados_internos, dados_iniciais)
                    self.after(0, self.aplicar_filtro)
            ...
            if dados_frescos and dados_frescos != self.tabela.dados_completos:
                ...
                self.after(0, self.tabela.atualizar_dados_internos, dados_frescos)
                self.after(0, self.aplicar_filtro)
    ...
    def aplicar_tema_completo(self):
        ...
        botoes = [self.btn_consultar, self.btn_csv, self.btn_excel, self.btn_alternar_tema, self.btn_alternar_ordem, self.btn_anterior, self.btn_proximo, self.btn_limpar_filtro]
    ...
    def consultar_api(self):
        ...
        try:
            ...
            self.after(0, self.tabela.atualizar_dados_internos, dados)
            self.after(0, self.aplicar_filtro)
    ...
    def aplicar_filtro(self):
        """
        Função central que lê o estado da UI e manda a tabela renderizar.
        """
        termo = self.entry_filtro.get()
        coluna = self.combo_coluna.get()
        self.tabela.renderizar_tabela(termo, coluna, ordem_desc=self.ordem_desc)
        self.atualizar_label_pagina()
